...
const TRANSLATIONS = {
  zh: {
    ...
    rule_title_policy: "⚠️ 住宿取消政策與付款", rule_sub_refund: "📅 取消退費標準", ref_14: "14天前", ref_desc_14: "退 100% (扣手續費) 或改期", ref_10: "10-13天前", ref_desc_10: "退 70% (2日內補差額)", ref_7: "7-9天前", ref_desc_7: "退 50%", ref_4: "4-6天前", ref_desc_4: "退 25%", ref_1: "1-3天前", ref_desc_1: "不退費",
...
const CAMPING_CONFIG = {
  ...
  room: { rates: { weekday: 2600, weekend: 2800, holiday: 2800, cny: 2800 }, discountType: "percentage" },
...
function calculateTotal() {
  ...
  if (nights >= 7 && !type.includes('full') && type !== 'car_bed_vip') {
      let lsConfig = { 'tent': {7: 3000, 14: 5000, 21: 5500, 30: 6000}, 'car': {7: 3000, 14: 5000, 21: 5500, 30: 6000}, 'solo': {7: 3000, 14: 5000, 21: 5500, 30: 6000}, 'moto': {7: 3000, 14: 5000, 21: 5500, 30: 6000},
                       'camper': {7: 4000, 14: 6500, 21: 7500, 30: 9000}, 'starcraft': {7: 8600, 14: 12000, 21: 12500, 30: 13000}, 'dt392': {7: 8400, 14: 11900, 21: 12000, 30: 12000}, 'room': {7: 9100, 14: 14000, 21: 16500, 30: 19800} };
  ...
  if (!isLongStay) {
      for (let i = 0; i < nights; i++) {
        ...
        if (i === 0 && isNightRush && config.nightRush) {
            ...
            if (type === 'room' && dayOfWeek === 0 && !['cny', 'holiday'].includes(rateType)) { rate_room = CAMPING_CONFIG.room.rates['weekend']; }
  ...
  if (type === 'car_bed_vip' || isLongStay) {
      ...
      if      (nights >= 5) discount = totalPriceForDiscount * 0.20;
      else if (nights === 4) discount = totalPriceForDiscount * 0.15;
      else if (nights === 3) discount = totalPriceForDiscount * 0.10;
      else if (nights === 2) discount = totalPriceForDiscount * 0.05;
  ...
  const isRentalType = ['starcraft', 'dt392', 'room'].includes(type);
  if (isRentalType && !isLongStay && (qty > 1 || extraPeople > 0)) {
      discount += totalPriceForDiscount * 0.05;
  } else if (!isRentalType && qty > 1 && !isLongStay && type !== 'car_bed_vip' && !type.includes('full')) {
      discount += totalPriceForDiscount * 0.05;
  }
